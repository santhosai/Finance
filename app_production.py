...
def get_person_details(name):
    ...
    # Calculate paid weeks based on payment ratio
    if amount_given > 0 and balance_amount > 0:
        total_original_weeks = int(amount_given / (balance_amount / balance_weeks)) if balance_weeks > 0 else 0
        paid_weeks = int((amount_paid / amount_given) * total_original_weeks)
    else:
        paid_weeks = 0
    ...
    details = {
        ...
        'total_weeks': paid_weeks + int(balance_weeks)
    }
